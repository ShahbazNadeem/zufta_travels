        ...
        <div className="container">
          ...
          <div className="history-content flex items-center gap-3 flex-wrap">
            {dashboardhistory.map((item) => (
              <div
                className=" basis-[100%] md:basis-[48%] h-[40vh] md:h-[50vh] lg:h-[50vh]  relative"
                key={item.id}
              >
                <figure
                ...
                <div className=" flex flex-col lg:gap-5 py-2 md:py-3 lg:py-3 justify-between absolute bottom-0 px-3">
                  ...
                  <div className=" px-4 text-sm my-1 text-[#DBDBDB]">
                    <span className="font-manrope">{item.time}</span>
                    <span className="px-1 font-manrope">from {item.price}</span>
                  </div>
                </div>
                <span className="absolute top-2 right-2 text-xs md:text-sm font-manrope bg-[#FFFFFFB2] rounded-full py-2 px-3 ">
                  {item.room}
                </span>
              </div>
            ))}
