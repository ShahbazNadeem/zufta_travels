...
const CatchAllDetailPage = () => {
  ...
  const id = slug[3] ?? null;
  console.error("url id", id);
  console.error("slug", slug);
  const [tourData, setTourData] = useState({});

  useEffect(() => {
    if (slug[0] !== "search" && slug[0] !== "tour") {
      router.push("/");
    }
  }, []);
  ...
  const fetchTourDetails = async () => {
    try {
      ...
      const response = await axios.get(
        `https://192.168.100.177:45455/api/Tours/TourDetailsById?id=${id}`
      );
  ...
  if (Array.isArray(slug) && slug[0]?.startsWith("tour")) {
    return (
      <Layout>
        <Head>
        ...
        <div>
          <div className="sticky top-26 bg-[#f8f8f8] max-w-[1300px] mx-auto px-5">
            <ul className="flex items-center gap-5 text-[14px] md:text-[16px] text-black">
              <li
                className={`p-3 sm:p-5 border-b-4 ${
                ...
                <div>
                  <div className="mt-10 md:mt-10 flex flex-col md:flex-row items-center justify-between gap-5 md:gap-0">
                    <h2 className="text-center md:text-left">
                      {tourData?.name || "Tour Name Appear Here"}
                    </h2>
                    {id}
                    <div className="flex items-center justify-between gap-8">
                      <span className="flex items-center gap-3">
                        <IoIosStar className="textColor2 text-lg md:text-xl lg:text-2xl" />
                        <span className="text-[23px] font-manrope font-bold textColor2">
                          4.3
                        </span>
                        <p>2034 ratings</p>
                      </span>
                  ...
                  <div className="flex items-center justify-between flex-wrap md:gap-0 gap-5 pt-5 md:pt-10">
                    <div className="md:basis-[48%] basis-[100%]">
                      <h3 className="mb-3 text-center md:text-left">
                        About this tour
                      </h3>
                      <p className="text-center md:text-left">
                        ...
                        autem At praesentium voluptatem aut libero nisi. Et
                        eligendi sint ab cumque veritatis aut provident aliquam.
                        Aut aspernatur consequuntur eum quaerat distinctio ut
                        inventore aliquid et quasi alias ut rerum suscipit et
                        nihil deleniti. Ex optio sequi et quos praesentium in
                        nostrum labore nam rerum iusto aut magni nesciunt? Quo
                        quidem neque iste expedita est dolor similique ut quasi
                        maxime ut deserunt autem At praesentium voluptatem aut
                        libero nisi. Et eligendi sint ab cumque veritatis aut
                        provident aliquam. Aut aspernatur consequuntur eum
                        quaerat distinctio ut inventore aliquid et quasi alias
                    ...
                    <div className="flex gap-5">
                      ...
                      <span
                        ...
                        className={`w-16 h-8 flex items-center px-1 rounded-full transition-colors duration-300 ${
                          isToggled ? "bg-gray-300" : "bg-gray-300"
                        }`}
                      >
                        <div
                  ...
                  <div className="flex justify-center  flex-wrap gap-2 mt-5 md:mt-10">
                    {pricingHondaBrv?.map((item, index) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className={`md:basis-[48%] xl:basis-[23%] p-1 md:p-5 hover:shadow-lg transition ${
                            !isLast
                              ? "border-r border-color"
                              : "xl:border-none border-r border-color"
                          }`}
                        >
                          ...
                          <p className="font-semibold mb-3 mt-6 md:mt-10">
                            Rs {item.price}
                          </p>
                          <p className="text-sm border-y border-color py-2">
                            {item.hotel}
                          </p>
                        </div>
                      );
